...
export class Storage extends EventTarget {
		...
		if (listenToGlobalEvents) {
			this.#listener = event => {
				if (event.storageArea == this.#backend && (event.key == null || event.key.startsWith(this.#keyPrefix)))
					this.dispatchEvent(new StorageEvent(event.key ? event.key.slice(this.#keyPrefix.length) : null, event.oldValue, event.newValue));
			};
	 ...
	 */
	get keys() {
		const keys = /** @type {string[]} */ (new Array(this.#backend.length).map((_, index) => this.#backend.key(index)));
